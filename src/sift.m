...
%Parameter
maxKeypoints = 10000000;
rect =[100 100 200 400];
row=256; 
colum=256; 

%imgOrigin=imread('E:\sample.jpg'); % muss verändert werden!
imgOrigin=imread('C:\Users\12400952\Desktop\test.png');

%save scales to translate keypoint locations in the end
...
% figure;
% %testimg = imread('E:\ippr\sample.jpg');
% imshow(origin);
% hold on
% disp(strcat('size',int2str(size(extrema,2))));
% plot(cy,cx, 'r+');
% for i = 1:extr_volume%size(extrema,2)
%     zeile = abs(extrema(i)/256);
%     spalte = mod(extrema(i),256);
%      %   zeile=floor((extrema(4*(i-1)+3)-1)/(256/(2^(extrema(4*(i-1)+1)-2))))+1;
%     %spalte=mod((extrema(4*(i-1)+3)-1),256/(2^(extrema(4*(i-1)+1)-2)))+1;
%     %disp(strcat(int2str(zeile),',',int2str(spalte)));
%    % plot(spalte,zeile, 'r+');
%     
% end
